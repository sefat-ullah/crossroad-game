...
void display()
{
	...
	float r,c;
	for(r=0;r<=abc.width;r++)      //read pixel(width) 
	{
		for(c=0;c<=abc.height;c++)    //read pixel(height)			
		{
			glBegin(GL_POINTS);
			float rr=(float)(abc.pixel(r,c,0));  //red color in a pixel
			float gg=(float)(abc.pixel(r,c,1));  // green color in a pixel
			float bb=(float)(abc.pixel(r,c,2));  // blue color in a pixel
			glColor3f(rr/255.0,gg/255.0,bb/255.0);  // combination of the color
			glVertex2f(r+20,c+15);      // write the pixel in (20,15) you can set pixel in a specific location
			glEnd();
		}
	}
	glutSwapBuffers();
